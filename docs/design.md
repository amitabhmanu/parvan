...
| D-12 | G-9: an attested absence must carry a re-runnable `silence:` record with at least one positive control | An argument from silence is the strongest thing the store produces and the easiest to fake, because a search that finds nothing looks exactly like a search that cannot find anything - which is what the `yavana` retraction was. Prose in the locus was accurate but not checkable. Structure splits the check in two: the loader verifies the record is complete, which works with no texts on disk, and `parvan verify-silences` re-runs the patterns and the controls against the live corpora, which is where silent drift shows up. A silence with unexplained hits is refused too: the two `dhṛtarāṣṭrī` hits in the Rāmāyaṇa are a bird-ancestress in a genealogy of geese, and a record that reported "0" was quietly wrong about its own numbers. |
